...
static void PrintMenuLine(WINDOW*, int, int, const std::string&);
static void PrintLine(WINDOW*, int, int, const char* line, std::size_t);
...
static void DisplayVersion()
{
  ...
  if (snprintf(
    ...
    ;
  MvAddStr(LINES - 2, (unsigned) (COLS - std::strlen(version)) >> 1, version);
}
...
{
  std::size_t i;
  std::size_t p = std::strchr(line.c_str(), '(') ? 2 : 0;
  std::size_t l = COLS + 2 + p;
  auto buffer = static_cast<char*>(std::malloc(l));
...
)
{
  ...
  if (!(buffer = static_cast<char*>(malloc(len + 2))))
  {
    ERROR_MSG("Malloc failed*ABORT");
    std::exit(EXIT_FAILURE);
    return;
